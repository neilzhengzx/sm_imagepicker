...
//    }
    ...
    throws FileNotFoundException{
        ...
        while(Math.max(imageHeight, imageWidth) /options.inSampleSize > Math.max(height, width) ){
        ...
        {
            scaleHeight = scaleWidth = (float)height/bmp.getHeight();
        }
        else
        {
            scaleHeight = scaleWidth = (float)width/bmp.getWidth();
        }
