...
- (void)initNavigationBar{
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemCancel target:self action:@selector(imageEditCancelBtn)];
...
-(void)imageEditCancelBtn{
    [self.navigationController dismissViewControllerAnimated:YES completion:^{
        if([self.delegate respondsToSelector:@selector(imageDidFinishEdittingCancel:)]){
            [self.delegate imageDidFinishEdittingCancel:_originalImage];
        }
    }];
}
